...
struct FVector {
    ...
    FVector(const VECTOR3& vec) : X(vec.X), Y(vec.Y), Z(vec.Z) {}

    // ✅ NEW: Assignment from VECTOR3
    FVector& operator=(const VECTOR3& vec)
    {
        X = vec.X;
        Y = vec.Y;
        Z = vec.Z;
    ...
    FVector GetSafeNormal() const {
        float S = Size();
        if (S < 1e-6f) return FVector(0, 0, 0);
