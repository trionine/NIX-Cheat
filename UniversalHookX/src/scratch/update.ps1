...
$replacements = @{}
$newOffsets -split "`n" | ForEach-Object {
    $line = $_.Trim()
    if ($line -match '^const\s+DWORD\s+(\w+)\s*=\s*(0x[0-9A-Fa-f]+);') {
        $name = $matches[1]
        $val = $matches[2].ToLower()
        $replacements[$name] = $val
    }
}

$filePath = "e:\LB TOOLS\SRC\4.4 Stealth - NIX HACK\UniversalHookX\src\offset.h"
$lines = Get-Content $filePath -Encoding UTF8
$newLines = @()

foreach ($line in $lines) {
    if ($line -match '^(\s*const\s+DWORD\s+)(\w+)(\s*=\s*)(0x[0-9a-fA-F]+)(.*)') {
        $prefix = $matches[1]
        $name = $matches[2]
        $eq_space = $matches[3]
        $old_val = $matches[4]
        ...
        if ($replacements.ContainsKey($name)) {
            ...
            $newLine = -join ($prefix, $name, $eq_space, $new_val, $suffix)
            $newLines += $newLine
        } else {
            $newLines += $line
        }
...
Write-Host "Successfully updated offsets."
