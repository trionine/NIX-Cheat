...
std::wstring Ws_ExType::ConvertToFAGlyphs(const std::wstring& text)
{
    ...
    bool fChar = false;
    for (const auto& ch : reversed_text)
    {
        if (IsFAChar(ch))
        {
            fChar = true;
    ...
    for (int i = 0; i < reversed_text.size(); ++i)
    {
        ...
        previous_fachar = (i - 1 >= 0 && IsFAChar(reversed_text[i - 1])) ? reversed_text[i - 1] : L"None";
        next_fachar = (i + 1 < reversed_text.size() && IsFAChar(reversed_text[i + 1])) ? reversed_text[i + 1] : L"None";
...
std::string Ws_ExType::ConvertToFAGlyphsS(const std::wstring& text)
{
    std::vector<std::wstring> reversed_text = ReverseFAText(text);
