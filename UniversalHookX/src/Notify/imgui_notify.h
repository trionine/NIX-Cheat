    ...
    NOTIFY_INLINE VOID RenderNotifications( ) {
        ...
        for (auto i = 0; i < notifications.size( ); i++) 
        {
            ...
            Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);
            {
                ...
                float slant = 10.0f;
                ImDrawList* bgdraw = GetWindowDrawList();
                ImVec2 bp1(wpos.x + slant,        wpos.y);
                ImVec2 bp2(wpos.x + wsz.x,        wpos.y);
                ImVec2 bp3(wpos.x + wsz.x - slant, wpos.y + wsz.y);
                ImVec2 bp4(wpos.x,                 wpos.y + wsz.y);
                ImVec2 bpts[4] = { bp1, bp2, bp3, bp4 };
                ImU32 bgFill   = ImColor(30, 30, 30, (int)(opacity * 230));
                ImU32 borderClr = ImColor((int)(text_color.x * 255), (int)(text_color.y * 255), (int)(text_color.z * 255), (int)(opacity * 200));
                bgdraw->AddConvexPolyFilled(bpts, 4, bgFill);
                bgdraw->AddPolyline(bpts, 4, borderClr, ImDrawFlags_Closed, 1.5f);
            }
            {
